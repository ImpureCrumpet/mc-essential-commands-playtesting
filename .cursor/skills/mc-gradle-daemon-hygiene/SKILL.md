...
This skill owns JDK, daemon, Loom, heap, and wrapper hygiene in **this** repo (including **`./gradlew --stop`** after a JDK switch). It does not own wiping `~/.gradle` caches or deleting other clones' `build/` trees.
...
Defer shared Java and source-set wiring with `pluginManager.withPlugin("java") { }` before touching `JavaPluginExtension` on subprojects. The callback runs as soon as the Java plugin is present, regardless of whether Loom applies it before or after the root configures the subproject. Avoid `afterEvaluate`: it is order-sensitive and can run too late for other Gradle configuration.
...
- [ ] `pluginManager.withPlugin("java") { }` used for shared Java wiring in root build script.
