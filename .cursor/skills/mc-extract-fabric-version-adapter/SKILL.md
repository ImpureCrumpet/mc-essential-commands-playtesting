...
- "Recipe registration differs across the 1.21.2 API boundary" → define `RecipeRegistrationAdapter` in root `src/`, implement the old serializer API under `mc1.21.1/`, and the codec-based API under `mc1.21.4/` and newer modules. (`1.21.2` / `1.21.3` are the API boundary, not supported targets — they have no row in `.cursor/fabric-loader-yarn-fabric-api.yaml`.)
