...
- **Default stop:** After **Phase 1** (`mc1.21.11` + `mc26.2`) is green, **do not** propose **Phase 2** or older **1.21.x** scaffolds unless the **user asks** or states a need (pack pins, wide compatibility). The guide **§3** treats the full ladder as **opt-in**.

## 1.21.x version coverage (phased rollout)

**Default:** anchor only — **`mc1.21.11`** + **`mc26.2`**. Supported targets are exactly the rows in **`.cursor/fabric-loader-yarn-fabric-api.yaml`**; **1.21.2** / **1.21.3** have no row. **26.3** has a ship row; it is not the default scaffold/test anchor. Optional wider matrix: **§3 — 1.21.x Version Coverage Strategy** in the guide.

| Phase | Subprojects | When |
|-------|-------------|------|
| **1 — Anchor** | `mc1.21.11` + `mc26.2` | First scaffold; usual completion point for bulk work |
...
- **`.cursor/fabric-loader-yarn-fabric-api.yaml`** — required authority for every coordinate and mapping choice; guide §3 is its human-readable projection (anchor needs 1.21.11 + 26.2 only).
...
- **§ Multi-Version Automated Testing** — load **`mc-fabric-gametest`** (`configureTests` / `@GameTest` / `fabric-gametest` entrypoint).
...
Resolve both **`loom_plugin_id`** and **`loom_version`** from the target's row in **`.cursor/fabric-loader-yarn-fabric-api.yaml`**, then apply that exact pair in this file. Do not use the legacy bare **`fabric-loom`** ID: 1.21.x uses **`net.fabricmc.fabric-loom-remap`**, while 26.x uses **`net.fabricmc.fabric-loom`**.
...
- **Mappings:** Resolve `mappings` from the target matrix row. For `yarn`, use its exact `yarn_mappings` coordinate (with the normal `:v2` classifier); for `mojmap`, follow the 26.x non-remapping setup. Do not substitute Mojang mappings for a row locked to Yarn.
- **Versions:** Use the target row's exact `loom_plugin_id`, `loom_version`, `loader_version`, and `fabric_api_version`. If the target has no row, stop: it is unsupported. A `mc-fabric-version-up` handoff does not override this gate or authorize coordinate promotion. Coordinate promotion is separate build-support maintenance: verify upstream values, update the YAML authority and derived guide, and update supported-version policy/tests together before resuming scaffolding.
...
### 6. Testing scaffold — load **`mc-fabric-gametest`**

Per guide §4 and **`mc-fabric-gametest`**:

- Wire Loom **`configureTests { eula = true }`**, **`fabric-gametest-api-v1`**, a minimal **`@GameTest`** smoke taking **`GameTestHelper`**, and the **`"fabric-gametest"`** entrypoint in **this** subproject’s **`fabric.mod.json`**.
- If a gametest class already exists, only add the **entrypoint** (and module dep / `configureTests` if missing) for the new subproject.
- Unit tests needing Fabric Loader, Minecraft bootstrap, registries, or mapped game classes → **`mc-fabric-loader-junit`** (not part of every scaffold). Pure helpers, math, parsers, and registry-free codecs stay on the repository's **ordinary JUnit** setup.
...
- [ ] Loom plugin ID and version match the target's matrix row (remapping ID for 1.21.x; non-remapping ID for 26.x).
...
- [ ] Gametest wired per **`mc-fabric-gametest`** / guide §4 (`configureTests`, entrypoint, smoke `@GameTest`).
...
When changing or evaluating this skill's description or triggers, use [references/trigger-evals.json](references/trigger-evals.json); do not load it during ordinary scaffolding.
