...
*v4.3.9 — **Portable bundle:** this file plus **`.cursor/skills/`** and the required coordinate matrix **`.cursor/fabric-loader-yarn-fabric-api.yaml`** copied into a mod repo (26.2 default 26.x anchor; **26.3** ship row added 2026-09-19). Toolchain: **SDKMAN!** (**`j21` / `j25`**), **no Foojay resolver plugin**, Gradle daemon vs toolchain hygiene, IDE/CI notes. Older guide snapshots: **git history** or local archive only.*

*Covers: multi-version **`mc*`** layout, **§3 default = anchor only** (`mc1.21.11` + `mc26.2`); optional wider 1.21.x ladder with **API band caveats**; 26.x transition (de-obfuscation + versioning).*
...
| **Completed current version → next version** (pinned handoff, API preparation, regression checks) | `.cursor/skills/mc-fabric-version-up/SKILL.md` |
| Mod Menu metadata, config screens (Cloth Config, MidnightLib) | `.cursor/skills/mc-fabric-modmenu-integration/SKILL.md` |
| **Mod SemVer bump** during dev playtest/rebuild (`mod_version`, `fabric.mod.json`, JAR names) | `.cursor/skills/mc-fabric-mod-version-bump/SKILL.md` |
| **Fabric Loader JUnit** unit tests (Loader, Minecraft bootstrap, registries, mapped game classes; pure helpers stay on ordinary JUnit) | `.cursor/skills/mc-fabric-loader-junit/SKILL.md` |
| **Fabric game tests** (server smoke / optional client; Loom `configureTests`) | `.cursor/skills/mc-fabric-gametest/SKILL.md` |

**Required coordinate authority:** **`.cursor/fabric-loader-yarn-fabric-api.yaml`** — the single source of truth for supported targets, loader, Loom plugin ID/version, Fabric API, and mapping type. The §3 table is a human-readable projection and must match the YAML. Copy both files with the portable bundle. **Rename note:** older copies used **`Fabric_Loader_Yarn_Fabric API.yaml`** (space in filename); use the kebab-case name above in scripts and deploy flows.

This guide and the **`mc-*`** skills under **`.cursor/skills/`** are standalone in the mod repo. Do not name or follow skills, registries, or paths from a sibling workspace.
...
- **Gradle 9.4.0** (required as of 26.1 Snapshot 1); **26.3** uses **9.6.0** (Fabric 26.3 article, full three-part wrapper URL). Resolve per matrix row / version-up lock. Run `./gradlew wrapper --gradle-version <exact>` with the three-part version.
- **Loom 1.15+** minimum for 26.1; **1.17.12** is locked for **26.2**; **1.17.21** is locked for **26.3** (see **`.cursor/fabric-loader-yarn-fabric-api.yaml`**). Do not lock `1.17-SNAPSHOT`.
- **Fabric Loader:** **`0.18.6`** on **1.21.x** subprojects; **`0.19.3`** on **26.x** through **26.2**; **`0.19.5`** on **26.3**. Loader is forwards-compatible within a major line — do not mix older 0.18.x on 26.x modules.
...
- **Supply chain:** [Aikido Safe Chain](https://github.com/AikidoSec/safe-chain) wraps **npm, pnpm, uv, pip** at the machine level — it does **not** cover Gradle/Maven. It protects Python tooling in this workspace. Managed via your dev-environment stack; do not run `safe-chain setup` as a per-clone step.
...
- **Root `build.gradle.kts`:** defer shared Java / source-set wiring with `pluginManager.withPlugin("java") { }`. The callback runs when each subproject's Java plugin is present and avoids `afterEvaluate`, which is order-sensitive and too late for some Gradle configuration.
...
The root `subprojects` block handles only shared repositories, Java toolchains, and source sets. **Do not declare or apply Loom at the root:** the remapping and non-remapping plugin markers resolve to the same implementation module at different versions, so a shared root plugin classpath can collapse them to one version. Each version module owns its exact Loom plugin ID and version.
...
### Version `build.gradle.kts` (`mc26.2/` — the 26.x anchor)

On **26.x**, `modImplementation` is retired in favor of standard Gradle configurations. Note: Obfuscation is gone at runtime, but mappings (Mojmap) are still layered internally by Loom for IDE support and parameter names.

Coordinates below are the **26.2** row. Resolve the row for the line you are actually building from **`.cursor/fabric-loader-yarn-fabric-api.yaml`** — do not copy these literals into an `mc26.1/` or `mc26.1.2/` module.
...
dependencies {
    ...
    implementation("net.fabricmc.fabric-api:fabric-api:0.152.2+26.2") // 26.2 row; resolve per 26.x line
}
...
Use this in a **temporary migration worktree/project** to generate Mojmap-named source as input to a 26.x port. The shipped 1.21.x module remains on the mapping type and coordinate locked by its YAML row; do not change that module to Mojmap. Loom exposes Gradle tasks; there is no separate plugin beyond **Fabric Loom**.
...
| **Before Gradle edits** | Follow the upstream migration flow only in the temporary copy. Do not replace the shipped `mc1.21.x` module's locked Yarn declaration. |
| **Output** | Default output is **`remappedSrc/`**. Use it as reference/input for 26.x-specific implementations; do not merge it wholesale over the shipped 1.21.x source tree. |
| **Yarn → 26.x path** | **1.21.11** is the last release where Yarn is published. Run `migrateMappings` against that version in the temporary copy, then move only the resulting 26.x implementation/adapters into `mc26.x/`; keep shared root code mapping-neutral. |
...
### Default: anchor only (`mc1.21.11` + `mc26.2`)

For **repos using this guide**, **stop here**: **`mc1.21.11`** (latest obfuscated 1.21.x line) and **`mc26.2`** (current 26.x anchor). Do **not** schedule, scaffold, or plan **Phase 2 / Phase 3** (older 1.21.x patch lines) unless the **user asks** or a mod has a **documented need** (e.g. pack pins to 1.21.8).
...
|-------|--------------|------|
| **Phase 1 — Anchor** | `mc1.21.11` + `mc26.2` | Always the first milestone; validate shared code, adapters, mixins across the 26.x boundary. |
...
- **1.21** (Gradle **`minecraft("1.21")`**, folder e.g. **`mc1.21`**) — **optional Phase 3** for the first **1.21** release before **1.21.1**. Use the exact **`+1.21`** Fabric API coordinate in the locked YAML row. Include **`mc1.21`** when you want a full ladder from the initial **1.21** through **1.21.11** without a one-off prompt.
- **1.21.2 / 1.21.3** — **not supported targets.** Short-lived releases whose Fabric API support stopped early, so they have **no row** in **`.cursor/fabric-loader-yarn-fabric-api.yaml`** and no coordinates to resolve. If a player asks, say they are not shipped rather than scaffolding a module. (`1.21.2` remains a real **API boundary** — see API band caveats below.)
...
**In this repository:** add a short **Supported Minecraft versions** section to **`README.md`** that matches reality:
...
Use when you **add** a row beyond the anchor. For **default** work you only need **1.21.11** and **26.2** scaffold rows; the other **26.x** lines (**26.1**, **26.1.1**, **26.1.2**) remain supported **ship** targets using the locked coordinates in **`.cursor/fabric-loader-yarn-fabric-api.yaml`**. Set `depends.fabric-api` to `>=` the exact coordinate. For a deliberate re-bump, verify against [Fabric Develop](https://fabricmc.net/develop) or [Maven](https://maven.fabricmc.net/net/fabricmc/fabric-api/fabric-api/), update the YAML first, then update this derived table in the same change.

**Supported targets:** **1.21**, **1.21.1**, **1.21.4**–**1.21.11**, **26.1**, **26.1.1**, **26.1.2**, **26.2**, **26.3**. A version is supported only if it has a row in **`.cursor/fabric-loader-yarn-fabric-api.yaml`**; **1.21.2** / **1.21.3** deliberately have none.

**Scope — mods, not servers.** This matrix governs **mod build targets**. What a **game server** runs is a separate, human decision (community mod coverage, whether the dot release is worth it, maintainer preference), and **holding an older line is legitimate**. Never propose a server upgrade because a version appears here.
...
| **1.21.7** | `0.129.0+1.21.7` | 3 | Gap fill |
...
| **1.21** | `0.102.0+1.21` | 3 | Initial **1.21** (before 1.21.1). **Optional** gap-fill; use the locked YAML row. |
| **26.1** | `0.145.1+26.1` | Ship | First 26.x line (de-obfuscation); loader **0.19.3**, Loom **1.17.12** |
| **26.1.1** | `0.145.2+26.1.1` | Ship | 26.1 patch line |
| **26.1.2** | `0.152.1+26.1.2` | Ship | Common LOREal ship target; often `mc26.1.2/` |
| **26.2** | `0.152.2+26.2` | 1 (Anchor) | Current default 26.x scaffold/test anchor; `mc26.2/` |
| **26.3** | `0.161.0+26.3` | Ship | Wilderness Bound; loader **0.19.5**, Loom **1.17.21**, Gradle **9.6.0**; not the default anchor |

**Loader version:** **`0.18.6`** on **1.21.x**; **`0.19.3`** on **26.x** through **26.2**; **`0.19.5`** on **26.3**. Do not use 0.18.x loader coordinates on 26.x subprojects.

**26.3 (ship):** Supported **mod** target with a verified matrix row (2026-09-19). It is **not** the default scaffold/test anchor and **not** a server-upgrade instruction. Preview/snapshot lines (e.g. **26.3-snapshot-***, **26.3-rc-***) stay out of the locked matrix. Further coordinate promotion remains separate build-support maintenance.

---

## 4. Multi-Version Automated Testing

**Skills:** **`mc-fabric-gametest`** (server/client game tests — default for load/mixin smoke) and **`mc-fabric-loader-junit`** (unit tests that need Fabric Loader, bootstrap, registries, or mapped game classes — pure helpers stay on ordinary JUnit). Official overview: [Fabric Automated Testing](https://docs.fabricmc.net/develop/automatic-testing).

**Default LOREal scaffold (game tests):** Loom `configureTests { eula = true }`, `fabric-gametest-api-v1`, a minimal `@GameTest` smoke class, and `"fabric-gametest"` in each version’s `fabric.mod.json`. `FabricGameTestHelper` was **removed** from Fabric — it is not an alternate scaffold; treat any surviving class implementing it as legacy in a pre-existing repo and see **`mc-fabric-gametest`**.

```bash
# Test ALL versions simultaneously (when tasks are wired)
./gradlew runGametest
```
...
**Unit tests:** only when non-trivial shared logic exists — **`mc-fabric-loader-junit`**. **Client game tests:** opt-in only (see gametest skill).
...
1. **Foundation (Phase 1):** Add root `settings.gradle.kts` and `build.gradle.kts`. Create `mc1.21.11/` and `mc26.2/`. Verify `./gradlew :mc1.21.11:runGametest` and `./gradlew :mc26.2:runGametest`.
...
> **Historical migration reference:** This section retains the 1.21.x → 26.x mapping/build checklist. Use the specific scaffold, adapter, mixin, recipe, and daemon skills for those mechanics. The old migration router is retired.
...
**`main` is the only branch you need.** The `mc26.2/` folder *is* the version isolation mechanism. A bug found in `mc1.21.11` gets fixed in the `mc1.21.11/` folder on `main`, built, and tagged.

1. On `main`, add `mc26.2/` subproject folder.
...
5. Build `:mc26.2`, tag, and publish.

---

## 8. 26.1 Migration & Environment Checklist

> For a completed current-version mod moving to the pinned next version, use **`mc-fabric-version-up`**. That workflow does not cover the historical 1.21.x mapping transition described below.
...
- [ ] **Loom:** Each module uses the plugin ID and version from its matrix row: **`net.fabricmc.fabric-loom-remap`** / **`1.15-SNAPSHOT`** for 1.21.x, **`net.fabricmc.fabric-loom`** / **`1.17.12`** for 26.x.
...
**26.x project work** (anchor `mc26.2`; substitute another supported 26.x line only when a mod targets it)

- [ ] If Mojmap-renamed source would help the 26.x port, run **`migrateMappings`** only in a temporary copy as described in §2. Keep the shipped 1.21.x module on its locked Yarn row; move mapping-specific output into `mc26.x/`, not shared root or `mc1.21.x/`.
- [ ] Create `mc26.2/` subproject
- [ ] Ensure the 26.x `build.gradle.kts` uses `implementation` instead of `modImplementation`
...
- [ ] Run `./gradlew :mc26.2:runGametest`
- [ ] Publish to Modrinth with game version `26.2`

---

## Document history

- **v4.3.9 (2026-09-19):** Added a verified **26.3** ship row (`0.161.0+26.3`, loader **0.19.5**, Loom **1.17.21**, Gradle **9.6.0**, JDK **25**). Default scaffold/test anchor remains **`mc1.21.11` + `mc26.2`**. Existing 26.2 and earlier rows unchanged.
- **v4.3.8 (2026-08-16):** Portable bundle is standalone in the consuming repo. This guide and **`mc-*`** skills no longer name sibling-repo registries or non-**`mc-*`** skill prefixes.

- **v4.3.7 (2026-08-08):** Made **`.cursor/fabric-loader-yarn-fabric-api.yaml`** the required single coordinate authority, with §3 explicitly derived from it. Aligned the 1.21.11 Gradle example to its Yarn row (`1.21.11+build.4:v2`), directed re-bumps to update YAML first, and confined Yarn→Mojmap migration output to a temporary porting copy so the shipped 1.21.x row remains Yarn.
- **v4.3.6 (2026-08-08):** Corrected the 1.21.7 Fabric API pin to **`0.129.0+1.21.7`** and added a regression that compares every human-table API coordinate with the machine matrix.
- **v4.3.5 (2026-08-08):** Split Loom by Minecraft era and keep plugin ownership module-local: 1.21.x modules use explicit **`net.fabricmc.fabric-loom-remap`**, while 26.x modules use **`net.fabricmc.fabric-loom`**. The coordinate matrix now records `loom_plugin_id` per row. The root neither declares nor applies Loom, preventing Gradle from collapsing both markers onto one implementation version; shared Java/source-set configuration uses `pluginManager.withPlugin("java")` instead of `afterEvaluate`.
- **v4.3.4 (2026-08-08):** **Supported target set is now exactly the matrix rows.** Removed `1.21.2` / `1.21.3` rows from **`.cursor/fabric-loader-yarn-fabric-api.yaml`** (short-lived; Fabric API support stopped early) so the machine authority no longer contradicts §3's skip policy; `1.21.2` remains an **API band** boundary in prose. **Default scaffold/test anchor moved from `mc26.1` to `mc26.2`** across §3, §7.3, §8, the per-version Gradle template, `AGENTS.md`, `mc-scaffold-new-fabric-subproject`, and `mc-fabric-gametest`; `26.1` / `26.1.1` / `26.1.2` stay supported **Ship** rows. **`26.3`** recorded as **upcoming** — no row, no invented coordinates, do not scaffold. New `tests/test_supported_versions.py` pins matrix ↔ §3 table ↔ anchor surfaces together.
- **v4.3.3 (2026-08-04):** § Cursor skills table adds **`mc-fabric-mod-version-bump`** (dev SemVer bump before playtest JAR rebuilds). Closes [Issue #16](https://github.com/ImpureCrumpet/Minecraft-Minder/issues/16). Repo version **3.37.0**.
- **v4.3.2 (2026-07-11):** § Cursor skills table adds **`mc-fabric-modmenu-integration`** (Mod Menu metadata, Cloth Config, MidnightLib). Closes [Issue #8](https://github.com/ImpureCrumpet/Minecraft-Minder/issues/8). Repo version **3.23.1**.
